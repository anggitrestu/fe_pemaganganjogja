import Renderitem from 'components/home/ListMagang/Renderitem';

function DetailPerusahaan({ data }) {
  return (
    <>
      <div className="card border-[1px] border-[#DFDFDF] border-solid text-[#404040]">
        <div className="card-body">
          <h2 className="font-semibold text-4xl text-bermuda mb-10">
            {data?.name}
          </h2>
          <h3 className="font-semibold text-2xl">Informasi Perusahaan</h3>
          <div className="mt-9">
            <h4 className="font-medium text-lg">Alamat</h4>
            <p>{data?.alamat}</p>
          </div>
          <div className="mt-8 flex">
            <div className="w-1/2">
              <h4 className="font-medium text-lg">Website</h4>
              <p>{data?.website}</p>
            </div>
            <div className="w-1/2">
              <h4 className="font-medium text-lg">Jenis Usaha</h4>
              <p>{data?.jenis_usaha}</p>
            </div>
          </div>
          <div className="mt-8 flex">
            <div className="w-1/2">
              <h4 className="font-medium text-lg">Jumlah Karyawan</h4>
              <p>{data?.jumlah_karyawan}</p>
            </div>
            <div className="w-1/2">
              <h4 className="font-medium text-lg">Ruang Pelatihan</h4>
              <p
                className={
                  'w-fit px-4 py-2 rounded-full ' +
                  (data?.is_room
                    ? 'text-[#36B752] bg-[#36b75242]'
                    : 'text-[#b73636] bg-[#b7363642]')
                }
              >
                {data?.pelatihan ? 'Tersedia' : 'Tidak Tersedia'}
              </p>
            </div>
          </div>
          <div className="mt-4 flex">
            <div className="w-1/2">
              <h4 className="font-medium text-lg">
                Instruktur yang bersertifikat metodologi pelatihan
              </h4>
              <p
                className={
                  'w-fit px-4 py-2 rounded-full ' +
                  (data?.is_instructor
                    ? 'text-[#36B752] bg-[#36b75242]'
                    : 'text-[#b73636] bg-[#b7363642]')
                }
              >
                {data?.is_instructor ? 'Tersedia' : 'Tidak Tersedia'}
              </p>
            </div>
          </div>
        </div>
      </div>
      <div className="text-[#404040] mt-8 mb-6">
        <h3 className="font-semibold text-2xl">Lowongan Magang</h3>
        <div className="grid grid-cols-2 gap-6 mt-6">
          {data?.internships?.length > 0 ? (
            data.internships.map((item, index) => {
              return <Renderitem item={item} key={index}></Renderitem>;
            })
          ) : (
            <div className="w-full text-center py-12">No Item Found</div>
          )}
        </div>
      </div>
    </>
  );
}

export default DetailPerusahaan;
