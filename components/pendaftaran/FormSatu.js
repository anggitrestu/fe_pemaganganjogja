...
function FormSatu() {
    ...
    return (
        <>
        {
            onClick && <Modal setOnClick={setOnClick}></Modal>
        }
        <div>
            ...
            <form className='mt-8'>
                <input name="lowongan-1" type="hidden" required/>
                <a onClick={()=>setOnClick(true)} className="flex border-[1px] bg-[#DFDFDF] hover:scale-[1.008] cursor-pointer transition-all text-sm text-[#8F8F8F] rounded-3xl py-5 px-8 mb-6">
                    <p className="mr-auto text-base">Pilih Lowongan 1</p>
                    <Image src="/images/search.svg" height={24} width={24} />
                </a>

                <input name="lowongan-2" type="hidden" required/>
                ...
                <input name="lowongan-3" type="hidden" required/>
